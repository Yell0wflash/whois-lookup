...
const PORT = process.env.PORT || String(Math.floor(Math.random() * 9)) + String(Math.floor(Math.random() * 9)) + String(Math.floor(Math.random() * 9)) + String(Math.floor(Math.random() * 9))
app.use(bodyParser());
...
console.log("[ EXPRESS ] Your app listen on port " + listener.address()?.port || 443);
